...
public class AdjacencyListSeamFinder implements SeamFinder {
    ...
    private class PixelGraph implements Graph<Node> {
        ...
        public PixelGraph(Picture picture, EnergyFunction f) {
            ...
            this.f = f;
            this.pixels = new Pixel[picture.width()][picture.height()]; //create 2d array w/ size [width] x [height]
            //fill in array 
            for (int y = 0; y < picture.height(); y += 1) {
                Pixel from = new Pixel(picture.width() - 1, y); //create new pixel at the very last row of pixels, and y height
                from.addEdge(new Edge<>(from, sink, 0)); //set neighbors to created node??
                pixels[picture.width() - 1][y] = from; //index this in 2d array
            }
            //fill in array 
